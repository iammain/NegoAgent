	...
	public Actions determineAcceptability() 
	{
            try {
                ...
                b = negotiationSession.getOpponentBidHistory().getLastBidDetails().getMyUndiscountedUtil() 
                		* 10 * (negotiationSession.getOwnBidHistory().getFirstBidDetails().getMyUndiscountedUtil()
                				- negotiationSession.getOpponentBidHistory().getFirstBidDetails().getMyUndiscountedUtil()) * (1 - negTime);
                ...
                if(lambda != 0)                
                    lambda += (TDO.getUniqueBidsCount()/negotiationSession.getOpponentBidHistory().size()) * (1 - lambda) * Math.pow(numbidInTime * dt, c);
                else
                	lambda = initlambda + (1 - initlambda) * Math.pow(discountFactor, b);
                                            
                double maxUtility = negotiationSession.getOwnBidHistory().getLastBidDetails().getMyUndiscountedUtil();
            ...
            if(bH.size() > 1){
                ...
                if(isLeapOfFaith(oppBid.getMyUndiscountedUtil(), prevOppBid.getMyUndiscountedUtil()))
                    if(oppBid.getMyUndiscountedUtil() >= getLeapThreshold())
        ...
        public boolean isLeapOfFaith(double bidUtil, double prevBidUtil){
            ...
            return difference * getTimeMultiplier() > .1;
        }
        public double getSigmoidTime() { return negotiationSession.getTime() * 10 - 5; }
        
        public double getTimeMultiplier() { return 1/(Math.pow(1 + Math.exp(-getSigmoidTime()), 0.3)); }
        
        public double getFinishTimeMultiplier() { return 1/(Math.pow(1 + Math.exp(getSigmoidTime()), 0.3)) + .66; }
