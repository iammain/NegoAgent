...
\author{Chatzichristodoulou Panagiotis , Tumanov Kirill \\ \it{Examiners:} \\Dr. Weiss, Gerhard}    % Declares the author's nam
...
The agents created are based on the negotiation platform genius. ~\cite{genius} . The genius platform released it's first demo edition from Delft University on May 14th 2009. It introduces an environment where agents can negotiate over different domains,  discrete or continuous. Genius is the standard in the agent negotiation domain platform.
...
\begin{figure}[h!]
	  \caption{Cooperation between four models,\newline \it{Picture taken from ANAC 2013 paper}}
	  \centering
...
This paper addresses implementation of an intelligent negotiation agent as well as  discussion and analysis of experiments performed are result obtained. Agent was develop based on the \textit{Genius} software platform in accordance with the \textit{ANAC'2013} guidelines and within the \textit{BOA} (Bidding Strategy, Opponent Model, Acceptance Strategy) framework. This work was performed as a practical part of the Multiagent Systems course at Maastricht University. 
