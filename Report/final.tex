...
The acceptance threshold is implemented so as to make the agent as hard-headed as possible. This makes the agent behave non optimally against opponents with the same strategy as his. For this reason, a sigmoid function\\
...
\begin{figure}[htbp]
  ...
  \centering
    ...
    \label{sigmoid}
\end{figure}
The threshold multiplier is taken from piecewise of the lower parts of the two functions, and it modifies $\xi$ according to its value. \\Although having this threshold makes the agent agree on bids that it would not just with the acceptance threshold , having this threshold increases the average utility the agent receives by 5-10$\%$.
...
if(isLeapBigEnough() && lastOpponentBidUtil > getTau()){
    Accept();
}
